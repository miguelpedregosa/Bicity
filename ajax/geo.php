...
if(isset($GET['latitud']) && isset($GET['longitud']))
 {
	 $salida = '';
	 $estaciones_json = array();
	 $buscar = new Search();
	 $cercanas = $buscar->cercanas((float)$GET['latitud'], (float)$GET['longitud'], null , 8);
	 foreach($cercanas as $estacion)
	 {
		 $est = new Station($estacion['city'], $estacion['number']);
		 $estacion_array = $est->to_array();
		 $estaciones_json[] = $estacion_array;
...
';
	 ...
	 $output = array();
	 $output['html'] = $salida;
	 $output['estaciones'] = $estaciones_json;
